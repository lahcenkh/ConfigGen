...
from configgen.ui import (
    highlight_rules,  # noqa: E402
    theme,  # noqa: E402
)
...
def _isolated_qsettings(tmp_path, monkeypatch):
    """theme.py, ui/settings.py, and highlight_rules.py all persist via
    QSettings(APP_ORG, APP_NAME), which is backed by the real Windows
    registry (or platform equivalent) by default — without this, running
    the test suite writes to and reads from the *actual developer/CI
    machine's* settings store, so a stale value from one run bleeds into
    and pollutes the next."""
    ...
    monkeypatch.setattr(highlight_rules, "_settings", fake_settings)
